...
$refunds = $pdo->query("SELECT SUM(total_amount) FROM transactions WHERE type='REFUND' AND status='PAID'")->fetchColumn() ?: 0;
...
<main class="flex-1 p-8 overflow-y-auto">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-10">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
            <p class="text-slate-400 text-[10px] font-bold mb-1 uppercase tracking-widest">Total Refund</p>
            <h3 class="text-xl font-bold text-slate-800"><?php echo formatCurrency($refunds); ?></h3>
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
            <p class="text-slate-400 text-[10px] font-bold mb-1 uppercase tracking-widest">Pending</p>
            ...
            <div class="mt-4 flex items-center text-slate-500 text-sm font-medium">
                <span class="w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center mr-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
